/* ==========================================================================
   Массово отклонить с комментарием
   ========================================================================== */
...
const rejectReplicaMany = async (req, res) => {
  const errors = [];
  ...
  if (!req.body.msg) {
    errors.push("Комментарий не указан");
  }

  if (!req.body.ids || (req.body.ids && req.body.ids.length < 1)) {
    errors.push("Выберите реплики, чтобы отклонить их");
  } else {
    ...
    if (replica && replica.length) {
      ...
      for (const item of replica) {
        ...
        if (replicaMsg) {
          ...
          replicaMsg.status = statusesList.refused;
...
module.exports = rejectReplicaMany;
