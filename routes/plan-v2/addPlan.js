...
const updateReplica = async (req, res) => {
	...
	if (errors.length) {
		...
		if (data && data.length) {
			for (const planStart of data) {
				const type = (planStart.type ? planStart.type : null)

				if (planStart.data) {
					for (const item of planStart.data) {
						if (item.dateStart && item.dateEnd) {
											...
											if (domain) {
												...
												if (itemDetail.id) {
													...
													if (planDetailUpdate && planDetailUpdate._id && planDetailUpdate._id.toString()) {
														...
														planDetailUpdate.url = (itemDetail.name ? itemDetail.name : null)
												...
												if (!updateDetail) {
													const planDetail = new PlanDetail({
														...
														plan: item.id,
														domain: domain,
														url: (itemDetail.name ? itemDetail.name : null),
							...
							if (!update) {
								...
								if (planId && item.items && item.items.length) {
									for (const itemDetail of item.items) {
										const domain = (itemDetail.name && getURL(itemDetail.name) ? getURL(itemDetail.name) : false)
